...
Процедура СписокНоменклатурыНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, Выполнение)
	...
	Значение = ЗначениеПеретаскиваияТовара(ДанныеСтроки); 
...
Процедура ПозицииПродажиПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Значение = ПараметрыПеретаскивания.Значение; //см. ЗначениеПеретаскиваияТовара
...
&НаСервере
Процедура СоздатьДекорацииИзбранныхТоваров()  // Содаем и переносим популярные товары в таблиц при нажатии
	...
	Для Сч = 1 По КоличествоИзбранныхТоваров Цикл
		...
		Декорация = Элементы.Добавить(ИмяДекорации, Тип("ДекорацияФормы"), Элементы.ГруппаПопулярныйПокупки);
		Декорация.Вид = ВидДекорацииФормы.Надпись;
		Декорация.Видимость = Ложь;
		Декорация.Гиперссылка = Истина; 
		Декорация.УстановитьДействие("Нажатие","Подключаемый_ДекорацияИзбранныхТоваровНажание");
				
		СтрокаОписаниДекорации = ДинамическиеЭлементы.Добавить();
		СтрокаОписаниДекорации.ИмяЭлемента = ИмяДекорации;
...
Процедура ЗаполнитьДекорациюБаллыКлиента()
	...
	ЧастиФормастированнойСтроки = Новый Массив; //Массив из Строка
	ЧастиФормастированнойСтроки.Добавить(Описание);
	ЧастиФормастированнойСтроки.Добавить(Новый ФорматированнаяСтрока("Заполнить", , , ,СсылкаЗаполненияБаллов ));
	
	Элементы.ДекорацияБаллыКлиента.Заголовок = Новый ФорматированнаяСтрока(ЧастиФормастированнойСтроки);
КонецПроцедуры

// Подключаемый декорация избранных товаров нажание.
// 
// Параметры:
//  Элемент - ДекорацияФормы
&НаКлиенте
Процедура Подключаемый_ДекорацияИзбранныхТоваровНажание(Элемент)
